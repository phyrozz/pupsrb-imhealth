...
export async function GET() {
  try {
    ...
    // Update reminder_sent flags on all users whose last assessment was >two weeks ago and part of the email sending batch
    await supabase
      .from("assessment_reminders")
    ...
    //
    // Send unanswered assessment email reminders
    const threeWeeksAgo = new Date(Date.now() - 21 * 24 * 60 * 60 * 1000).toISOString()
    const { data: unansweredUsers, error: unansweredDBError } = await supabase
      .from("assessment_reminders")
      .select(`id, created_at, last_assessment_at, reminder_sent, profiles (personal_details (user_id, email, first_name))`)
    ...
    if (unansweredDBError) {
      throw unansweredDBError
    }

    const unansweredEmailsToSend = []

    for (const user of unansweredUsers) {
      if (unansweredEmailsToSend.length <= 20) {
        // Check if user has personal details and email
        if (user.profiles && user.profiles.personal_details && user.profiles.personal_details.length > 0) {
          const email = user.profiles.personal_details[0].email
          const firstName = user.profiles.personal_details[0].first_name || 'there' // Default to 'there' if first name is not available
          unansweredEmailsToSend.push({
            userId: user.profiles.personal_details[0].user_id,
            email: {
              from: "team@pupsrc-otms.online",
              to: [email],
    ...
    await supabase
      ...
      .eq("reminder_sent", true)
      .eq("unanswered_reminder_sent", false)
      .in("user_id", unansweredEmailsToSend.flatMap(email => email.userId))

    await resend.batch.send(unansweredEmailsToSend.flatMap(email => email.email))
